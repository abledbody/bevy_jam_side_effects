...
pub fn record_controls(
	...
	for mut mob_inputs in &mut player_query {
		...
		if input_resource.pressed(KeyCode::A) { movement.x -= 1.0 }
		if input_resource.pressed(KeyCode::D) { movement.x += 1.0 }
		if input_resource.pressed(KeyCode::W) { movement.y += 1.0 }
		if input_resource.pressed(KeyCode::S) { movement.y -= 1.0 }
